...
function extractTable(element: HTMLElement, topic: string, extractDepth: boolean, columns: number): string[][] {
  ...
  if (body === null) throw new Error(`No table body found for: ${topic}`);
...
function addParameters(table: HTMLElement, path: string, type: string, columns: { count: number; name: number; required: number; description: number; example: number; default?: number }): OpenAPIV3.ParameterObject[] {
  return extractTable(table, path, false, columns.count)
    .map((d) => {
      ...
      const description = d[columns.description];
...
(async () => {
  ...
  right.querySelectorAll("h2").forEach((element) => {
    function getNextElement(tagName: string, skip = 0): HTMLElement {
      do {
        if (element.nextElementSibling === null) throw new Error(`Could not find ${tagName} after ${element.tagName}`);
    ...
    function processParam(field: FoxESSField, parent: OpenAPIV3.SchemaObject): void {
      ...
      if (field.isArray) {
        ...
        if (field.remark !== undefined) schema.description = field.remark;
    ...
    if (method !== OpenAPIV3.HttpMethods.GET) {
      ...
      indent(requestBody, path).forEach((p) => {
        processParam(p, bodyRoot);
      });
    }
    indent(getNextElement("TABLE"), path).forEach((p) => {
      processParam(p, responseRoot);
    });
  });
